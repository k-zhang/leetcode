package com.algomind.leetcode.mycalendar1;
...
class MyCalendarTest {
    private final MyCalendar myCalendar = new MyCalendar();

    @ParameterizedTest
    @MethodSource("provideData")
    public void test(boolean[] expected, int[][] event) {
        for(int i = 0 ; i < event.length ; i++) {
            Assertions.assertEquals(expected[i], myCalendar.book(event[i][0], event[i][1]));
        }
    }
